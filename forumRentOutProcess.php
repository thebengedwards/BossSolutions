...
<html lang="en">
    ...
    <?php
        ...
        $topicAddress = filter_has_var(INPUT_POST, 'topic_address') ? $_POST['topic_address'] : null;
        $topicAddress = trim($topicAddress);
        ...
        if ($_SESSION['loggedIn'] == true) {
            ...
            if ($checkAdmin == true || $checkGuest == true || $checkTenant == true) {
                ...
                if (strlen($topicAddress) > 100) {
                    echo "<p>Please enter an address with less than 100 characters</p>";
                }

                if ($errors === true) {
                    echo "<p>You didn't fill in the form correctly, please try <a href='forumRentOut.php'>again</a>.</p>\n";
                } else {
                    ...
                    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                        try {
                            ...
                            $stmt = $dbConn->prepare("INSERT INTO forum_threads (thread_cat, thread_title, topic_text, topic_by, topic_date, topic_address) 
                                                          VALUES (:thread_cat, :thread_title, :topic_text, :topic_by, :topic_date, :topic_address)");
                            ...
                            $stmt->bindParam(':topic_address', $topicAddress);
                            ...
                            $id = $_SESSION['userID'];
                            //$topicAddress = $_POST['topic_address'];
                            $stmt->execute();
