    ...
    if ($_SESSION['loggedIn'] == true)
    {
        if (checkPermission('Admin') == true)
        {
            ...
            $SQLquery = "SELECT amount, date
                 ...
                 LEFT JOIN Property ON Property.propertyID = Tenant.PropertyID
                 LEFT JOIN Landlord ON Property.landlordID = Landlord.landlordID
                 WHERE tenantTransaction.type = 1
                 ORDER BY date ASC
                         ";
            $stmt = $dbConn->prepare($SQLquery);
            $stmt->execute(array());
    ...
    if (checkPermission('Admin') == true)
    {
        ...
        $SQLquery = "SELECT SUM(amount) AS rentPaid
                     ...
                     WHERE tenantTransaction.type = 0 AND year(tenantTransaction.date) =:yearT
                         ";
        $stmt = $dbConn->prepare($SQLquery);
        $stmt->execute(array(':yearT' => $yearT));
        ...
        $SQLquery = "SELECT SUM(amount) AS utilPaid
                 ...
                 LEFT JOIN Account ON tenantTransaction.tenant_id_fk = Account.accountID
                     ...
                     LEFT JOIN Property ON Property.propertyID = Tenant.PropertyID
                     LEFT JOIN Landlord ON Property.landlordID = Landlord.landlordID
                     WHERE tenantTransaction.type = 1 AND year(tenantTransaction.date) =:yearT
                         ";
        $stmt = $dbConn->prepare($SQLquery);
        $stmt->execute(array(':yearT' => $yearT));
        ...
        $dbConn = getConnection();
        $SQLquery = "SELECT SUM(Property.rent) AS rent, SUM(Property.bills) AS bills
                     FROM Tenant
                     LEFT JOIN Property ON Property.propertyID = Tenant.PropertyID
                     LEFT JOIN Landlord ON Property.landlordID = Landlord.landlordID
                         ";
                            ...
                            <div class='trancTenantDashHolder' onmouseover='changeNameColor(\"trancTName1\")' onmouseout='changeNameColorBack(\"trancTName1\")'>
                                <div class='trancTenantDashHolder2'>
                                    <div class='trancTenantDashName' id='trancTName1'>
                                        <p style='margin-left: 10px; margin-top: 5px;'>Income Overview</p>
                                    </div>
                                    <div style='width: 100%;'>
                                        <table>
                                            <tr>
                                                <td><p>Upcoming Payments: </p></td>
                                                <td><a href='adminRent.php'><div class='trancTenantUpcomingHolder'><p>1st {$nextMonth} - £{$monthlyRent}</p></div></a></td>
                                                <td><a href='adminUtilities.php'><div class='trancTenantUpcomingHolder'><p>15th {$utilDate} - £{$monthlyUtil}</p></div></a></td>
                                            </tr>
                                            <tr>
                                                <td style='height: 100px;'><p>Total Due: £{$totalDue}</p></td>
                                                <td><p>Total Received: £{$totalPaid}</p></td>
                                    ...
                                    <div class='trancTenantDashHolder2'>
                                        <a href='adminRent.php'>
                                            <div class='trancTenantDashName' id='trancTName2'>
                                                    <p style='margin-left: 10px; margin-top: 5px;'>Rent Income</p><p id='transNameArrows2' style='margin-left: 120px; margin-top: -33px;'> >></p>
                                            </div>
                                        </a>
                                        ...
                                        <table style='background-color: transparent'>
                                            <tr>
                                                <td style='width: 30%; border-right: solid 1px #333333'>
                                                        <p style='font-size: 10px'>Next Rent Income:</p>
                                                        <p style='font-size: 10px'>1st {$nextMonth} - £{$monthlyRent}</p><hr style='border: solid 0.5px #333333'>
                                                        <p style='font-size: 10px'>Total Rent Income: £{$totalRent}</p>
                                                        <p style='font-size: 10px'>Total Rent Recieved: £{$rentPaidT}</p>
                                                        <p style='font-size: 10px'>Outstanding Income: £{$outRent}</p>  
                                                        <p style='font-size: 10px'>Income Due Monthly: £{$monthlyRent}</p>                                               
                    ...
                    <tr>
                        <td>
                            <div class='trancTenantDashHolder' onmouseover='changeNameColor(\"trancTName3\")' onmouseout='changeNameColorBack(\"trancTName3\")'>
                                    <div class='trancTenantDashHolder2'>
                                        <a href='adminUtilities.php'>
                                            <div class='trancTenantDashName' id='trancTName3'>
                                                <p style='margin-left: 10px; margin-top: 5px;'>Utilities Income</p><p id='transNameArrows2' style='margin-left: 120px; margin-top: -33px;'> >></p>
                                            </div>
                                        </a>
                                        <div class='trancTenantYearHolder'>
                        ...
                        <td>
                            <div class='trancTenantDashHolder' onmouseover='changeNameColor(\"trancTName4\")' onmouseout='changeNameColorBack(\"trancTName4\")'>
                                <a href='PropertyAdmin.php'><div class='trancTenantDashHolder2'>
                                    <div class='trancTenantDashName' id='trancTName4'>
                                        <p style='margin-left: 10px; margin-top: 5px;'>Manage Properties</p>
                                    </div>
                                    <div style='margin-left: 10px; margin-right: 10px; '>
                                        <p style='margin-left: 20px; margin-right: 20px; margin-top: 50px; text-align: center'>Click here to make changes to the rent and utility costs of all properties on the sytem.</p>
                                    </div>
...
<script type="text/javascript">
    ...
    function reloadPageWithDateBack()
    {
        ...
        const page = 'adminBills.php?rentGraphYear=';
