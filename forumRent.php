        ...
        if ($_SESSION['loggedIn'] == true) {
            ...
            if ($checkAdmin == true || $checkGuest == true || $checkTenant == true) {
                echo '
                    ...
                    <form action="forumRentProcess.php" method="post" class="forumPostForm">
            ...
            ';
                ...
                try {
                    ...
                    $stmtSQL = "SELECT accountID, thread_id, thread_title, thread_cat, username, topic_date, topic_text FROM forum_threads
                                JOIN Account on topic_by = accountID
                                
                                WHERE thread_cat = 'rent'
                    ...
                    while ($rowObj = $queryResult->fetchObject()) {
                        ...
                        if ($_SESSION['userID'] == $rowObj->accountID) {
                            ...
                            <div class=\"thread\">
                                <p class='threadTitle'>{$rowObj->thread_title}</p>
                                <h4>Content: </h4><p>{$rowObj->topic_text}</p>
                                <p>Posted By: <span class='postedBy'>{$rowObj->username}</span> on {$rowObj->topic_date}</p>
                                ";
